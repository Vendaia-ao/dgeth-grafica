...
import { Mail, MapPin, Phone, ArrowRight, Clock, CheckCircle, Loader2 } from 'lucide-react';
...
const Contact = () => {
  ...
  // ── Formulário Simples → WhatsApp ──────────────────────────────────────────
  const handleSubmitSimples = (e: React.FormEvent) => {
    e.preventDefault();
    const msg = encodeURIComponent(
      `Olá! Sou ${formData.name}${formData.email ? ` (${formData.email})` : ''}.\n\n${formData.message}`
    );
    // Extract phone number from whatsapp_url or phone field
    const waUrl = contacts.whatsapp_url || `https://wa.me/244944974378`;
    const base = waUrl.replace(/\?.*$/, ''); // strip any existing query
    window.open(`${base}?text=${msg}`, '_blank', 'noopener');
    setSubmitted(true);
    setTimeout(() => setSubmitted(false), 4000);
    setFormData({ name: '', phone: '', email: '', service: '', deadline: '', message: '', file: null });
  };

  // ── Formulário Orçamento → Supabase Leads ─────────────────────────────────
  const handleSubmitOrcamento = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.file) {
        ...
        if (!uploadErr) {
          const { data: pubData } = supabase.storage.from('uploads').getPublicUrl(path);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    if (activeTab === 'simples') {
      handleSubmitSimples(e);
    } else {
      handleSubmitOrcamento(e);
    }
  };
  ...
  return (
    <>
      <SEO
        pagePath="/contacto"
        defaultTitle="Contacto — Dgeth Gráfica"
        defaultDescription="Fale connosco para solicitar orçamentos e tirar dúvidas sobre os nossos serviços de impressão e comunicação visual."
      />
              ...
              {/* WhatsApp Card */}
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-foreground">{contacts.phone}</p>
                  </div>
                </a>
              ...
              {/* Email Card */}
                <div className="stagger-item flex items-center gap-4 group p-6 rounded-2xl bg-card shadow-sm border border-border">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-foreground break-all">{contacts.email}</p>
                  </div>
                </div>
              ...
              {/* Location Card */}
                ...
                <div className="stagger-item flex items-center gap-4 group p-6 rounded-2xl bg-card shadow-sm border border-border">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-foreground">{contacts.working_hours}</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end pt-4">
                  <button
                    ...
                    className={`w-full md:w-auto px-10 py-5 text-white font-bold text-lg rounded-xl transition-all shadow-lg transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 ${
                      submitted 
                        ? 'bg-green-600 shadow-green-600/30' 
                        : 'bg-ns-blue hover:bg-ns-dark shadow-ns-blue/30'
                    }`}
                  >
                    {submitting ? (
                      <>
